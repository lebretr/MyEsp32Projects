...
void setup() {
  ...
  for (int i=0; i<NumberOfDht; i++) {
    ...
    if(i==0){
      ...
      dhtTH_V[i].dht=new DHTRL(DHTPIN_0);
    }
    if(i==1){
      pinMode(DHTPIN_1, INPUT_PULLUP);
      dhtTH_V[i].dht=new DHTRL(DHTPIN_1);
    }
    if(i==2){
      pinMode(DHTPIN_2, INPUT_PULLUP);
      dhtTH_V[i].dht=new DHTRL(DHTPIN_2);
    }
    if(i==3){
      pinMode(DHTPIN_3, INPUT_PULLUP);
      dhtTH_V[i].dht=new DHTRL(DHTPIN_3);
    }
    if(i==4){
      pinMode(DHTPIN_4, INPUT_PULLUP);
      dhtTH_V[i].dht=new DHTRL(DHTPIN_4);
    }
    if(i==5){
      pinMode(DHTPIN_5, INPUT_PULLUP);
      dhtTH_V[i].dht=new DHTRL(DHTPIN_5);
    }
