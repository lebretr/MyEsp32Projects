...
ZigbeeNumber::ZigbeeNumber(uint8_t endpoint) : ZigbeeEP(endpoint) {
...
bool ZigbeeNumber::addBinaryInput() {
  ...
  _binary_clusters |= NUMBER_INPUT;
  return true;
}

// bool ZigbeeNumber::addBinaryOutput() {
...
bool ZigbeeNumber::setBinaryInputApplication(uint32_t application_type) {
  if (!(_binary_clusters & NUMBER_INPUT)) {
...
// bool ZigbeeNumber::setBinaryOutputApplication(uint32_t application_type) {
...
bool ZigbeeNumber::setBinaryInput(bool input) {
...
bool ZigbeeNumber::reportBinaryInput() {
...
bool ZigbeeNumber::setBinaryInputDescription(const char *description) {
  if (!(_binary_clusters & NUMBER_INPUT)) {
    log_e("Binary Input cluster not added");
    return false;
  }
...
// bool ZigbeeNumber::setBinaryOutputDescription(const char *description) {
//   if (!(_binary_clusters & BINARY_OUTPUT)) {
//     log_e("Binary Output cluster not added");
//     return false;
//   }
...
// void ZigbeeNumber::zbAttributeSet(const esp_zb_zcl_set_attr_value_message_t *message) {
...
// void ZigbeeNumber::binaryOutputChanged() {
...
// bool ZigbeeNumber::setBinaryOutput(bool output) {
//   esp_zb_zcl_status_t ret = ESP_ZB_ZCL_STATUS_SUCCESS;
...
// bool ZigbeeNumber::reportBinaryOutput() {
//   /* Send report attributes command */
//   esp_zb_zcl_report_attr_cmd_t report_attr_cmd;
//   report_attr_cmd.address_mode = ESP_ZB_APS_ADDR_MODE_DST_ADDR_ENDP_NOT_PRESENT;
